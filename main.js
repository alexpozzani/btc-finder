...
import { encontrarBitcoins, encontrarBitcoinsReverso } from './bitcoin-find.js'
...
rl.question(`Escolha uma carteira puzzle( ${chalk.cyan(1)} - ${chalk.cyan(161)}): `, (answer) => {
    ...
    rl.question(`Escolha uma opcao (${chalk.cyan(1)} - Comecar do inicio, ${chalk.cyan(2)} - Escolher uma porcentagem, ${chalk.cyan(3)} - Escolher minimo): `, (answer2) => {
        if (answer2 == '2') {
            ...
            rl.question('Entre o minimo: ', (answer3) => {
                ...
                max = BigInt(answer3)
                key = BigInt(min)
                encontrarBitcoinsReverso(key, min, max, () => shouldStop, answer - 1)
                rl.close();
            });
        } else {
            min = BigInt(min)
